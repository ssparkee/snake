...
def drawGrid(gridWidth=800, gridHeight=600):
    gridSize = (gridWidth//BLOCKSIZE, gridHeight//BLOCKSIZE)
...
def drawSnake(snake):
    for i in snake['body']:
        bodyRect = pygame.Rect(i[0], i[1], BLOCKSIZE, BLOCKSIZE)
        pygame.draw.rect(screen, (50, 125, 0), bodyRect)

    headRect = pygame.Rect(
        snake['head'][0], snake['head'][1], BLOCKSIZE, BLOCKSIZE)
...
def drawApple(applePos):
    ...
    pygame.draw.circle(screen, 
                        (255, 0, 0),
                        (applePos[0] + (BLOCKSIZE/2), applePos[1] + (BLOCKSIZE/2)), 
                        BLOCKSIZE/2.3)


def getRandApplePos(gridWidth=800, gridHeight=600):
    randPos = (
        (randint(1, gridWidth//BLOCKSIZE-1)*BLOCKSIZE),
        (randint(1, gridHeight//BLOCKSIZE-1)*BLOCKSIZE)
    )
...
snake = {'length': 3, 'head': (0, 3*BLOCKSIZE), 'body': [(
    0, BLOCKSIZE), (0, 2*BLOCKSIZE)], 'last': (0, 0), 'direction': (0, 0)
}
...
while True:
    ...
    if snake['head'][0] >= 800 or snake['head'][1] >= 600 or snake['head'][0] < 0 or snake['head'][1] < 0:
        print('game over')
        running = False
    if not running:
        break
